...
- **Building**: `lua/user/build.lua` — async `cmake --build` / `make` via `vim.system`, output parsed into the quickfix list with a custom `errorformat` (appended to the built-in one) and surfaced through Trouble. Keymaps are under `<leader>b`, registered by `M.setup()` from `init.lua`; the statusline component is consumed by `lualine.lua`. Three details are load-bearing: output is line-buffered per stream (chunks split mid-line would parse as bogus entries), `valid == 0` entries are filtered out after parsing (unmatched build chatter would otherwise fill the list), and `unsplice()` repairs interleaved lines before parsing. Multi-line linker/CMake messages are folded with `%+C` and flattened to one line.
  - **`unsplice()` exists because parallel builds share one stderr pipe.** Two compiler processes can interleave mid-line, welding a source-snippet line onto the front of the next diagnostic. Vim's errorformat matching is an unanchored *search*, so such a line still parses — but `%f` swallows the garbage prefix (it spans spaces, not just `isfname` characters) and the entry points at a path that does not exist; Trouble then opens an empty buffer and throws `Invalid cursor line: out of range` on jump. The weld lands on an ordinary path character, so no pattern can find the seam — the filesystem arbitrates, cutting at the first `/` whose tail is `filereadable`. Lines whose prefix ends in `from ` are exempt: `In file included from /path/a.h:1:0:` carries its path mid-line by design.
...
- **Running**: `lua/user/run.lua` — launches built executables with saved argument profiles from `.nvim-run.json` at the project root, streaming each run's output into its own log buffer (filterable by `[LVL]`). Several runs are supported concurrently, keyed by id in a `runs` table. It locates executables via `build.binary_dir()` rather than probing itself, so the preset resolution has exactly one home. Three non-obvious choices: recency ordering lives in `stdpath("cache")`, never in the committed profile file (launching would otherwise dirty the tree); `cwd` defaults to the project root, not the exe's directory, because game data resolves relative to the repo; and buffer appends are batched on a 60ms timer, since a verbose program emitting thousands of lines a second will stall the editor if each line touches the buffer. `vim.json.encode` turns an empty Lua table into `{}`, so profile arrays are serialised by hand. `<leader>bP` / `:Run!` launch a profile under `samply record` — samply must never be signalled directly mid-recording (it dies without writing the capture; observed empirically), so `M.stop` on a profiled run `pkill -INT`s the game binary and lets samply wind down; only when no game process matches does it kill the job (samply's post-run server phase).
- **Profile form**: `lua/user/form.lua` — a small field form in a floating window (`section`/`text`/`int`/`bool`/`enum`), used by `run.lua` to build run profiles. Two contracts matter: `on_submit` runs *before* the window closes and returning `false` keeps it open (closing first would discard everything typed whenever a save is rejected), and `required`/`int` validation happens in the form so the message can name the field. The label column is sized to the longest label at render time — a fixed width collided with the value column. A `choice` field picks from `values_fn()`, evaluated on activation so the list reflects current state; an entry whose value is `form.PROMPT` falls through to a text prompt so a picker is never a dead end.
  - **While the form float is focused, the current buffer is the form**, so anything resolving the project from the current buffer silently fails inside it. `build.binary_dir()` and the run module's executable lookups therefore take an explicit `root`, captured when the form opens. The symptom was the command preview rendering `<exe>` and the executable picker coming up empty.
  - Saving an edited profile merges through `merge_profile`, **not** `vim.tbl_extend`: a cleared field arrives as `nil`, a nil carries no key, so `tbl_extend("force", ...)` left the previous value in place and `exe`/`cwd`/`filter` could never be cleared. It assigns each key the form owns (`FORM_KEYS`) so nil removes it, while leaving unmanaged keys such as `env` untouched. Renaming migrates the profile's entry in the recency store, which is keyed by name.
